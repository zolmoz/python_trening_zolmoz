...
class ORMFixture:
    ...
    def convert_groups_to_model(self, groups):
        def convert(group):
            return Groupe(id=str(group.id), name=group.name, header=group.header, footer=group.footer)
    ...
    def convert_contacts_to_model(self, contacts):
        def convert(contact):
            return Contactfilld(id=str(contact.id), firstname=contact.firstname, lastname=contact.lastname,
    ...
    @db_session
    def get_contacts_in_group(self, Groupe):
        orm_group = list(select(g for g in ORMFixture.ORMGroup if g.id == Groupe.id))[0]
        return self.convert_contacts_to_model(orm_group.contacts)

    @db_session
    def get_contacts_not_in_group(self, Groupe):
        orm_group = list(select(g for g in ORMFixture.ORMGroup if g.id == Groupe.id))[0]
